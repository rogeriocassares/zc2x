...
func buildMessages() []Message {
	...
	msgs = append(msgs, pdmOutputBankMessages(0x401, 2, "OutputCurrent", "A", 30)...)
	msgs = append(msgs, pdmOutputBankMessages(0x405, 6, "OutputVoltage", "V", 30)...)
...
// pdmOutputBankMessages splits n indexed 8-bit unsigned 0.1-scale channels
// (PDM output current or voltage) into ceil(n/8) messages of up to 8
...
// than hand-typed: 30 near-identical channels x 2 (current+voltage) is
// genuinely repetitive, indexed data, same reasoning as
// ecu/main/main.c's encode_pd_bank_u8.
func pdmOutputBankMessages(startID uint32, startMsgNum int, sigNamePrefix, unit string, n int) []Message {
	var msgs []Message
	ch := 1
	for msgNum := startMsgNum; ch <= n; msgNum++ {
		...
		for j := 0; j < count; j++ {
			sigs[j] = Signal{Name: fmt.Sprintf("%s%d", sigNamePrefix, ch), ByteOffset: j, Type: Uint8, Scale: 0.1, Unit: unit}
			ch++
		}
...
var baseMessages = []Message{
	...
	{ID: 0x110, Name: "CD2", DLC: 8, Signals: []Signal{
		...
		{Name: "GForceLateral", ByteOffset: 2, Type: Uint16, Scale: 0.001, Bias: -5, Unit: "g"},
		{Name: "GForceLongitudinal", ByteOffset: 4, Type: Uint16, Scale: 0.001, Bias: -5, Unit: "g"},
	...
	// corner, 3-point IR array (inner/middle/outer across the tread) — see
	// the DBC's BO_ 304 comment for the MLX90614 sensor and why the encoding
	// (unsigned, 0.1 C/LSB, offset -40 = this message's physical minimum)
	// is standardized to match every other temperature signal here rather
	// than the sensor's own native 0.02 C/LSB register step.
	{ID: 0x130, Name: "TTUFL", DLC: 6, Signals: []Signal{
	...
	{ID: 0x140, Name: "TTUFR", DLC: 6, Signals: []Signal{
	...
	{ID: 0x150, Name: "TTURL", DLC: 6, Signals: []Signal{
	...
	{ID: 0x160, Name: "TTURR", DLC: 6, Signals: []Signal{
	...
	// PDM30/M130/C125/L180 custom re-aggregation -- see
	...
	// native CAN broadcast. PD2-PD15 (output/input current/voltage/status
	// banks) are generated by buildMessages() above, not listed here.
	{ID: 0x400, Name: "PD1", DLC: 7, Signals: []Signal{
